...
String getItemTypeCode() // используем sFormType
{
  ...
  if (idx<0)  idx = sFormType.lastIndexOf("Master");
...
//	if (sFormType.equals("Сб. единица Master")) innpaneDetal.saveToUserData();
//	if (sFormType.equals("Материал Revision Master")) innpaneMaterial.saveToUserData();
//	if (sFormType.equals("Документ Revision Master")) innerPanelDocument.saveToUserData();
//	if (sFormType.equals("Комплекс Revision Master")) innerPanelKompleks.saveToUserData();
//	if (sFormType.equals("Комплект Revision Master")) innerPanelKomplekt.saveToUserData();
//	if (sFormType.equals("Прочее Revision Master")) innerPanelProchee.saveToUserData();
//	if (sFormType.equals("Стандартное изд. Revision Master")) innerPanelStd_izd.saveToUserData();
	...
	if (sitem_form == null) {
		...
		sform.setFormPropertySafe(NR.fld_N_PROTRAZ, data.n_protraz+","+LUtil.Date2String(data.date_protocol) );
		Object gg = form.getUid();
		if (gg!=null) gg.toString();
		form.save();
		form.unlock();
	} else {
		item_form.lock();   
		...
		sitem_form.setFormPropertySafe(NR.fld_SPRAV_DSE, data.sprav_dse);
		sitem_form.setFormPropertySafe(NR.fld_FIRST_USE, data.first_use);
		sitem_form.setFormPropertySafe(NR.fld_UOBOZ_IZD, data.izdelie);
		sitem_form.setFormPropertySafe(NR.fld_TIPIZGOT, data.type_izgot);
		sitem_form.setFormPropertySafe(NR.fld_VIDPKI, data.vid_pki);
		sitem_form.setFormPropertySafe(NR.fld_CODEOKP, data.code_okp);
		sitem_form.setFormPropertySafe(NR.fld_DOC_POSTAV, data.doc_postav);
		sitem_form.setFormPropertySafe(NR.fld_ORG_POSTAV, data.org_postav);
		...
		sitem_form.setFormPropertySafe(NR.fld_N_PROTRAZ, data.n_protraz+","+LUtil.Date2String(data.date_protocol) );
		Object gg = form.getUid();
		if (gg!=null) gg.toString();
		item_form.save();
		item_form.unlock();
      ...
      if (sFormType.startsWith(NR.TYPE_Prochee+" "))
      {
        ...
        sform_rev.setFormPropertySafe(NR.fld_frommodel, data.pr_from_model );
...
{
	// добавлено Стеценко, если создается новый объет то item надо брать из формы визарда создания нового объекта
	if (data.item == null) {
		try {
			...
			ItemMasterFormPanel imfp1 = (ItemMasterFormPanel)scrolpan1.getParent();	
			NewItemPanel itemPanel1 = (NewItemPanel)imfp1.controlWizardPanel;	
			/*System.out.println(parentPanel.toString()+" | "+ enclosing.toString()+
				" | " +scrolpan1.toString()+ " | " + imfp1.toString() + 
				" | " + itemPanel1.toString());*/
  ...
  try {
    session.queueOperation(operation); // в этом случае мы не будет ждать конца операции
  }
  catch (Exception ex) {
    MessageBox.post(ex);
  }
...
public void loadRendering() throws TCException
{
  ...
  // -- Для Прочего основной формой считаем Item Master но надо подчитать массу из Revision  --
  form_rev = form;
  if (sFormType.equals(NR.TYPE_Prochee+" Master"))
  {
    form_rev = (TCComponentForm) data.item_rev.getRelatedComponent("IMAN_master_form_rev");
  }
  else if (sFormType.equals(NR.TYPE_Prochee+" Revision Master"))
  {
    form = (TCComponentForm) data.item.getRelatedComponent("IMAN_master_form");
  }
  data.form = form;
  ...
  {
    /*
      double Massa = form_rev.getFormTCProperty(NR.fld_MASSA).getDoubleValue();
      if (Massa<0)
      {
     ...
     data.massa = Massa;
    */
    data.pr_from_model = (Boolean)sform_rev.getFormPropertyObj(NR.fld_frommodel);
    data.massa = data.pr_from_model ?
      data.getMassFromModel() : (Double)sform_rev.getFormPropertyObj(NR.fld_MASSA);

    // Считывание комментария для Прочих
    data.comment = data.item.getProperty("object_desc");
    // Разработчик
    data.creator = (String)sform_rev.getFormPropertyObj(NR.fld_data2_R);
    // Раздел справочника
    data.material = (String)sform_rev.getFormPropertyObj(NR.fld_MATERIAL);
  }

  try{
    //data.code_dse = form.getFormTCProperty(NR.fld_DSE_CODE).getStringValue();
    data.indication = (String)sform.getFormPropertyObj(NR.fld_DSE);
    data.name_dse = (String)sform.getFormPropertyObj(NR.fld_NAME_DSE);
    data.first_use = (String)sform.getFormPropertyObj(NR.fld_FIRST_USE);
    data.sprav_dse = (String)sform.getFormPropertyObj(NR.fld_SPRAV_DSE);
    data.izdelie = (String)sform.getFormPropertyObj(NR.fld_UOBOZ_IZD);
    data.gost = (String)sform.getFormPropertyObj(NR.fld_GOST);
    //data.otvetstv = (String)sform.getFormPropertyObj(NR.fld_OTVETSTV);
    data.type_doc = (String)sform.getFormPropertyObj(NR.fld_TIP_DOC);
    data.type_izgot = (String)sform.getFormPropertyObj(NR.fld_TIPIZGOT);
    data.code_okp = (String)sform.getFormPropertyObj(NR.fld_CODEOKP);
    data.doc_postav = (String)sform.getFormPropertyObj(NR.fld_DOC_POSTAV);
    data.org_postav = (String)sform.getFormPropertyObj(NR.fld_ORG_POSTAV);
    ...
    String strArray[];
    strArray = LUtil.parse2Array((String)sform.getFormPropertyObj(NR.fld_N_PROTRAZ));
    data.n_protraz = strArray[0];
    data.date_protocol = LUtil.String2Date(strArray[1]);

    // Списки
    // (Ответственное подразделение)
    //data.lov_otvetstv = LUtil.getLOVStrings(NR.lov_OTVETSTV, session);
    // Вид ПКИ
...
//         || sFormType.equals("СЕ Станд. Master")  )
  ...
  else if (sFormType.equals(NR.TYPE_Prochee+" Master") || sFormType.equals(NR.TYPE_Prochee+" Revision Master"))
  {
...
/*    else if (sFormType.equals("Комплекс Master"))
  {
    innerPanel = new DetalItemPanel(data);
    innerPanel.setHeader("Комплекс");
  }
  else if (sFormType.equals("Тех. узел Master"))
  {
    innerPanel = new DetalItemPanel(data);
    innerPanel.setHeader("Технологический узел");
  }
  */
